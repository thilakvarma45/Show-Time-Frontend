...
const AppContent = () => {
  ...
  useEffect(() => {
    if (user?.id) {
      const token = localStorage.getItem('token');
      fetch(`https://show-time-backend-production-5dff.up.railway.app/api/wishlist/${user.id}`, {
        headers: {
  ...
  const handleEventBookNow = async (event) => {
    try {
      ...
      const res = await fetch(`https://show-time-backend-production-5dff.up.railway.app/api/events/${event.id}`);
      if (!res.ok) {
  ...
  const handlePaymentComplete = async (paymentInfo) => {
    try {
      ...
      if (bookingType === 'MOVIE') {
        const token = localStorage.getItem('token');
        response = await fetch('https://show-time-backend-production-5dff.up.railway.app/api/bookings/movie', {
          method: 'POST',
          headers: {
        ...
        const token = localStorage.getItem('token');
        response = await fetch('https://show-time-backend-production-5dff.up.railway.app/api/bookings/event', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
  ...
  const handleToggleWishlist = async (item) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch('https://show-time-backend-production-5dff.up.railway.app/api/wishlist/toggle', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  const handleRemoveFromWishlist = async (id, type) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await fetch(`https://show-time-backend-production-5dff.up.railway.app/api/wishlist/${user.id}/${type}/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
