...
const Settings = ({ user, onBack, onSave }) => {
  ...
  const uploadProfileImage = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://show-time-backend-production-5dff.up.railway.app/api/upload/profile/${user.id}`, {
        method: 'POST',
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://show-time-backend-production-5dff.up.railway.app/api/auth/user/${user.id}`, {
        method: 'PUT',
        headers: {
