...
const MovieListings = ({ onSelectShow, owner }) => {
  ...
  useEffect(() => {
    const loadScheduledMovies = async () => {
      ...
      try {
        ...
        const res = await fetch(`https://show-time-backend-production-5dff.up.railway.app/api/shows/summary?ownerId=${owner.id}`, {
  ...
  useEffect(() => {
    const loadEvents = async () => {
      ...
      try {
        const token = localStorage.getItem('token');
        const res = await fetch(`https://show-time-backend-production-5dff.up.railway.app/api/events/owner/${owner.id}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        if (!res.ok) {
  ...
  const handleDelete = async (e, item) => {
    ...
    try {
      ...
      let url = '';
      if (item.type === 'movie') {
        url = `https://show-time-backend-production-5dff.up.railway.app/api/shows/movie/${item.tmdbMovieId}?ownerId=${owner.id}`;
      } else {
