...
const VenueManagement = ({ owner }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = isEditing
        ? `https://show-time-backend-production-5dff.up.railway.app/api/venues/${editingVenueId}`
        : 'https://show-time-backend-production-5dff.up.railway.app/api/venues';
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await fetch(
        `https://show-time-backend-production-5dff.up.railway.app/api/venues/${confirmDeleteVenueId}${owner?.id ? `?ownerId=${owner.id}` : ''}`,
        {
          method: 'DELETE',
          headers: {
