...
const SmartScheduler = ({ owner }) => {
  ...
  useEffect(() => {
    const loadVenues = async () => {
      try {
        ...
        const res = await fetch(`https://show-time-backend-production-5dff.up.railway.app/api/venues/owner/${owner.id}`, {
          headers: {
  ...
  const handleMovieSubmit = async (e) => {
    ...
    try {
      ...
      const token = localStorage.getItem('token');
      const res = await fetch('https://show-time-backend-production-5dff.up.railway.app/api/schedules', {
        method: 'POST',
        headers: {
  ...
  const uploadEventImage = async () => {
    ...
    try {
      ...
      const token = localStorage.getItem('token');
      const response = await fetch('https://show-time-backend-production-5dff.up.railway.app/api/upload/event', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        },
      ...
      return `https://show-time-backend-production-5dff.up.railway.app${data.imageUrl}`;
    } catch (error) {
  ...
  const handleEventSubmit = async (e) => {
    ...
    try {
      ...
      const token = localStorage.getItem('token');
      const response = await fetch('https://show-time-backend-production-5dff.up.railway.app/api/events', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
