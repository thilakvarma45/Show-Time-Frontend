  ...
  useEffect(() => {
    const loadEventZones = async () => {
      ...
      try {
        ...
        const res = await fetch(`https://show-time-backend-production-5dff.up.railway.app/api/events/${item.id}`, {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const analyticsUrl = `https://show-time-backend-production-5dff.up.railway.app/api/analytics/${isMovie ? 'movie' : 'event'}/${idToUse}${item.ownerId ? `?ownerId=${item.ownerId}` : (owner?.id ? `?ownerId=${owner.id}` : '')}`;
        const bookingsUrl = `https://show-time-backend-production-5dff.up.railway.app/api/bookings/${isMovie ? 'movie' : 'event'}/${idToUse}${item.ownerId ? `?ownerId=${item.ownerId}` : (owner?.id ? `?ownerId=${owner.id}` : '')}`;
