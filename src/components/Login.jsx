...
const Login = ({ onAuthSuccess }) => {
  ...
  const backendSocialAuth = async (email, uid, name = '', phone = '') => {
    ...
    try {
      ...
      let res = await fetch('https://show-time-backend-production-5dff.up.railway.app/api/auth/google-login', {
        method: 'POST',
  ...
  const handleEmailLogin = async (e) => {
    ...
    try {
      ...
      const res = await fetch('https://show-time-backend-production-5dff.up.railway.app/api/auth/login', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
