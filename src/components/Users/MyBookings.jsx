...
const MyBookings = ({ user, onBack }) => {
  ...
  useEffect(() => {
    const loadBookings = async () => {
      try {
        ...
        const res = await fetch(`https://show-time-backend-production-5dff.up.railway.app/api/bookings/user/${user.id}`, {
  ...
  const confirmCancel = async () => {
    ...
    try {
      ...
      const res = await fetch(`https://show-time-backend-production-5dff.up.railway.app/api/bookings/${selectedBooking.id}/cancel`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
      });
