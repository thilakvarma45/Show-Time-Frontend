...
const MovieDetails = ({ onBookNow }) => {
  ...
  const loadRatings = async (movieId) => {
    try {
      ...
      const summaryRes = await fetch(`https://show-time-backend-production-5dff.up.railway.app/api/ratings/movie/${movieId}`);
      if (summaryRes.ok) {
      ...
      if (token) {
        const userRes = await fetch(`https://show-time-backend-production-5dff.up.railway.app/api/ratings/movie/${movieId}/user`, {
  ...
  const loadReviewsList = async () => {
    if (!movie?.id) return;
    try {
      const res = await fetch(`https://show-time-backend-production-5dff.up.railway.app/api/ratings/movie/${movie.id}/reviews`);
      if (res.ok) {
