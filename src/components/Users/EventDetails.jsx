...
const EventDetails = ({ onBookNow }) => {
    ...
    const loadRatings = async (eventId) => {
        try {
            ...
            const summaryRes = await fetch(`https://show-time-backend-production-5dff.up.railway.app/api/ratings/event/${eventId}`);
            if (summaryRes.ok) {
            ...
            if (token) {
                const userRes = await fetch(`https://show-time-backend-production-5dff.up.railway.app/api/ratings/event/${eventId}/user`, {
    ...
    const loadReviewsList = async () => {
        if (!event?.id) return;
        try {
            const res = await fetch(`https://show-time-backend-production-5dff.up.railway.app/api/ratings/event/${event.id}/reviews`);
            if (res.ok) {
    ...
    const submitRating = async () => {
        ...
        try {
            const res = await fetch('https://show-time-backend-production-5dff.up.railway.app/api/ratings', {
                method: 'POST',
                headers: {
    ...
    useEffect(() => {
        const loadEvent = async () => {
            ...
            setError(null);
            try {
                const res = await fetch(`https://show-time-backend-production-5dff.up.railway.app/api/events/${id}`);
                if (!res.ok) {
                ...
                if (firstDateId) {
                    try {
                        const availRes = await fetch(
                            `https://show-time-backend-production-5dff.up.railway.app/api/bookings/event/${full.id}/zone-availability?eventDateId=${encodeURIComponent(firstDateId)}`
                        );
                        if (availRes.ok) {
    ...
    useEffect(() => {
        const loadOtherEvents = async () => {
            try {
                const res = await fetch('https://show-time-backend-production-5dff.up.railway.app/api/events');
                if (!res.ok) {
